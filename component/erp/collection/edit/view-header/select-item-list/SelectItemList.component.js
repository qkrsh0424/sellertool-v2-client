...
export default function SelectItemListComponent({
    ...
    return (
        <>
            <Container>
                <GridContainer>
                    {refErpCollectionHeaders?.every(r => erpCollectionHeaderDetailsForm.some(r2 => r2.refErpCollectionHeaderId === r.id)) ?
                        <div
                    ...
                    {refErpCollectionHeaders?.map((r, index) => {
                        let isSelected = erpCollectionHeaderDetailsForm?.find(formHeader => formHeader.refErpCollectionHeaderId === r.id);
            ...
            <SelectedItemListContainer>
                {(!erpCollectionHeaderDetailsForm || valueUtils.isEmptyValues(erpCollectionHeaderDetailsForm)) ?
                    ...
                    <DragDropContext onDragEnd={(result) => onActionReorderForm(result)}>
                        <Droppable droppableId={uuidv4()}>
                            {provided => (
                                ...
                                >
                                    {erpCollectionHeaderDetailsForm?.map((r, index) => {
                                        return (
                                            <Draggable
                                                key={r.id}
                                                draggableId={r.id}
