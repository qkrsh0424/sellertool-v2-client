...
const RecordUpdateModalComponent = (props) => {
    const [nameState, dispatchNameState] = useReducer(nameStateReducer, initialNameState);

    useEffect(()=>{
        if(!props.marginDataControlState.name){
            return;
        }
        dispatchNameState({
            type:'SET_DATA',
            payload:props.marginDataControlState.name
        })
    },[props.marginDataControlState])
        ...
        <>
            <Container>
                ...
                <FooterWrapper>
                    <FooterBox>
                        <ButtonBox>
                            <button type='button' className='confirm-btn' onClick={(e) => _onConfirm(e)}>확인</button>
...
export default RecordUpdateModalComponent;