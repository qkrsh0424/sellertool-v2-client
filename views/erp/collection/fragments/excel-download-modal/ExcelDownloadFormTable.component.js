...
const TableBody = ({ refErpCollectionHeaders, header }) => {
    return (
        <tbody>
            <tr>
                {header.headerDetail.details.map((detail, index) => {
                    ...
                    let name = detail.viewDetails.map(r => {
                        let originCellName = refErpCollectionHeaders.filter(dr => dr.matchedColumnName === r.matchedColumnName)[0].originCellName;
