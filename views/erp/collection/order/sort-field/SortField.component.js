    ...
    return (
        <>
            <Container>
                ...
                >
                    <div>{FLATTEN_SORT_LIST?.find(r => r.sort === router?.query?.sort)?.name || '[M] 주문등록일'} | <span className='bold'>{FLATTEN_SORT_LIST?.find(r => r.sort === router?.query?.sort)?.direction || '오름차순'}</span></div>
                </SingleBlockButton>
            </Container>
            {sortModalOpen &&
