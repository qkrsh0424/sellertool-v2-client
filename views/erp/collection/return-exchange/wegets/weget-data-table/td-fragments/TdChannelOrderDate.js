...
export function TdChannelOrderDate({
    returnExchange
}) {
            ...
            returnExchangeActionsHook.returnExchangesActions.setValue(prev => {
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...prevItem,
                            channelOrderDate: newItem?.channelOrderDate,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = () => {
        setIsEditMode(true);
        setInputValues({
            channelOrderDate: returnExchange?.channelOrderDate || new Date()
        });
    }

    const handleSubmit = async (value) => {
        inputValuesRef.current.channelOrderDate = value;
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={() => handleClickOpenEditMode()}>{returnExchange?.channelOrderDate ? CustomDateUtils().dateToYYYYMMDDhhmmss(returnExchange?.channelOrderDate) : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
            ...
            {isEditMode &&
                <>
                    <CustomDateTimeSelector
                        ...
                        initialDateTime={new Date(inputValues?.channelOrderDate)}
                        label='날짜'
                    />
                </>
            }
