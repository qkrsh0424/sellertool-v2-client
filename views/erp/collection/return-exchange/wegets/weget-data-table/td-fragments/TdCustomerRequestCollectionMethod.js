...
export function TdCustomerRequestCollectionMethod({
    returnExchange
}) {
    ...
    const handleReqBulkUpdate = async () => {
        setIsLoading(true);
        const updateResult = await ReturnExchangeDataConnect().bulkUpdate({
            body: {
                returnExchanges: [{
                    ...returnExchange,
    ...
    const handleClickOpenEditMode = (e) => {
        ...
        setInputValues({
            customerRequestCollectionMethod: returnExchange?.customerRequestCollectionMethod,
        });
    }
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>{returnExchange?.customerRequestCollectionMethod ? returnExchange?.customerRequestCollectionMethod : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
                    ...
                    <EditBox>
                        <form onSubmit={(e) => handleSubmit(e)}>
                            <div className='inputBox'>
                                <div>
                                    <input type="text" name='customerRequestCollectionMethod' value={inputValues?.customerRequestCollectionMethod || ''} onChange={(e) => handleChangeInputValues(e)} autoFocus />
                                </div>
                            </div>
