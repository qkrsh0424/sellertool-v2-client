...
export function TdReceiverInfo({
    returnExchange
}) {
                ...
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...
                            receiverContact1: newItem?.receiverContact1,
                            receiverContact2: newItem?.receiverContact2,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = (e) => {
        ...
        setInputValues({
            receiver: returnExchange?.receiver,
            receiverContact1: returnExchange?.receiverContact1,
            receiverContact2: returnExchange?.receiverContact2,
        });
    }
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>수취인) {returnExchange?.receiver ? returnExchange?.receiver : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                    </div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>연락처1) {returnExchange?.receiverContact1 ? returnExchange?.receiverContact1 : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                    </div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>연락처2) {returnExchange?.receiverContact2 ? returnExchange?.receiverContact2 : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                    </div>
                    {isLoading &&
                        <FieldCircleLoading
                            progressSize={20}
                    ...
                    <form onSubmit={(e) => handleSubmit(e)}>
                        <EditBox>
                            <div className='inputBox'>
                                <div className="label">수취인)</div>
                                <div>
                                    <input type="text" name='receiver' value={inputValues?.receiver || ''} onChange={(e) => handleChangeInputValues(e)} autoFocus />
                                </div>
                            </div>
                            <div className='inputBox'>
                                <div className='label'>연락처1)</div>
                                <div>
                                    <input type="text" name='receiverContact1' value={inputValues?.receiverContact1 || ''} onChange={(e) => handleChangeInputValues(e)} />
                                </div>
                            </div>
                            <div className='inputBox'>
                                <div className='label'>연락처2)</div>
                                <div>
                                    <input type="text" name='receiverContact2' value={inputValues?.receiverContact2 || ''} onChange={(e) => handleChangeInputValues(e)} />
                                </div>
                            </div>
