...
export function TdReturnExchangeAppliedAt({
    returnExchange
}) {
            ...
            returnExchangeActionsHook.returnExchangesActions.setValue(prev => {
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...prevItem,
                            returnExchangeAppliedAt: newItem?.returnExchangeAppliedAt,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = () => {
        setIsEditMode(true);
        setInputValues({
            returnExchangeAppliedAt: returnExchange?.returnExchangeAppliedAt || new Date()
        });
    }

    const handleSubmit = async (value) => {
        inputValuesRef.current.returnExchangeAppliedAt = value;
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={() => handleClickOpenEditMode()}>{returnExchange?.returnExchangeAppliedAt ? CustomDateUtils().dateToYYYYMMDDhhmmss(returnExchange?.returnExchangeAppliedAt) : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
            ...
            {isEditMode &&
                <>
                    <CustomDateTimeSelector
                        ...
                        initialDateTime={new Date(inputValues?.returnExchangeAppliedAt)}
                        label='날짜'
                    />
                </>
            }
