...
export function TdReturnExchangeCollectionRequestedAt({
    returnExchange
}) {
    ...
    const [inputValues, setInputValues] = useState({
        returnExchangeCollectionRequestedAt: ''
    });
            ...
            returnExchangeActionsHook.returnExchangesActions.setValue(prev => {
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...prevItem,
                            returnExchangeCollectionRequestedAt: newItem?.returnExchangeCollectionRequestedAt,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = () => {
        setIsEditMode(true);
        setInputValues({
            returnExchangeCollectionRequestedAt: returnExchange?.returnExchangeCollectionRequestedAt || new Date()
        });
    }

    const handleSubmit = async (value) => {
        inputValuesRef.current.returnExchangeCollectionRequestedAt = value;
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={() => handleClickOpenEditMode()}>{returnExchange?.returnExchangeCollectionRequestedAt ? CustomDateUtils().dateToYYYYMMDDhhmmss(returnExchange?.returnExchangeCollectionRequestedAt) : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
            ...
            {isEditMode &&
                <>
                    <CustomDateTimeSelector
                        ...
                        initialDateTime={new Date(inputValues?.returnExchangeCollectionRequestedAt)}
                        label='날짜'
                    />
                </>
            }
