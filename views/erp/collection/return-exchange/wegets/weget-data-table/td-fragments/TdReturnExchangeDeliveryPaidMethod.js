...
export function TdReturnExchangeDeliveryPaidMethod({
    returnExchange
}) {
    ...
    const [inputValues, setInputValues] = useState({
        returnExchangeDeliveryPaidMethod: '',
    });
            ...
            returnExchangeActionsHook.returnExchangesActions.setValue(prev => {
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...prevItem,
                            returnExchangeDeliveryPaidMethod: newItem?.returnExchangeDeliveryPaidMethod,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = (e) => {
        ...
        setInputValues({
            returnExchangeDeliveryPaidMethod: returnExchange?.returnExchangeDeliveryPaidMethod,
        });
    }
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>{returnExchange?.returnExchangeDeliveryPaidMethod ? returnExchange?.returnExchangeDeliveryPaidMethod : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
                    ...
                    <EditBox>
                        <form onSubmit={(e) => handleSubmit(e)}>
                            <div className='inputBox'>
                                <div>
                                    <input type="text" name='returnExchangeDeliveryPaidMethod' value={inputValues?.returnExchangeDeliveryPaidMethod || ''} onChange={(e) => handleChangeInputValues(e)} autoFocus />
                                </div>
                            </div>
