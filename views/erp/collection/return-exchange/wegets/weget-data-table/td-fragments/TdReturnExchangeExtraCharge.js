...
export function TdReturnExchangeExtraCharge({
    returnExchange
}) {
            ...
            returnExchangeActionsHook.returnExchangesActions.setValue(prev => {
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...prevItem,
                            returnExchangeExtraCharge: newItem?.returnExchangeExtraCharge,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = (e) => {
        ...
        setInputValues({
            returnExchangeExtraCharge: returnExchange?.returnExchangeExtraCharge,
        });
    }
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>{returnExchange?.returnExchangeExtraCharge ? `${CustomNumberUtils().numberWithCommas2(returnExchange?.returnExchangeExtraCharge)}원` : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
                    ...
                    <EditBox>
                        <form onSubmit={(e) => handleSubmit(e)}>
                            <div className='inputBox'>
                                <div>
                                    <input type="number" name='returnExchangeExtraCharge' value={inputValues?.returnExchangeExtraCharge || ''} onChange={(e) => handleChangeInputValues(e)} autoFocus />
                                </div>
                            </div>
