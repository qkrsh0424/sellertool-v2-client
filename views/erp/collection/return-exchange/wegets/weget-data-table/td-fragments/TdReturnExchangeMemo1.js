...
export function TdReturnExchangeMemo1({
    returnExchange
}) {
            ...
            returnExchangeActionsHook.returnExchangesActions.setValue(prev => {
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...prevItem,
                            returnExchangeMemo1: newItem?.returnExchangeMemo1,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = (e) => {
        ...
        setInputValues({
            returnExchangeMemo1: returnExchange?.returnExchangeMemo1,
        });
    }
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>{returnExchange?.returnExchangeMemo1 ? returnExchange?.returnExchangeMemo1 : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
                    ...
                    <EditBox>
                        <form onSubmit={(e) => handleSubmit(e)}>
                            <div className='inputBox'>
                                <div>
                                    <textarea type="text" name='returnExchangeMemo1' value={inputValues?.returnExchangeMemo1 || ''} onChange={(e) => handleChangeInputValues(e)} autoFocus />
                                </div>
                            </div>
