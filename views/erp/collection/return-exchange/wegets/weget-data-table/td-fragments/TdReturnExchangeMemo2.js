...
export function TdReturnExchangeMemo2({
    returnExchange
}) {
            ...
            returnExchangeActionsHook.returnExchangesActions.setValue(prev => {
                return prev.map(prevItem => {
                    ...
                    if (newItem) {
                        return {
                            ...prevItem,
                            returnExchangeMemo2: newItem?.returnExchangeMemo2,
                        };
                    } else {
                        return prevItem;
                    }
    ...
    const handleClickOpenEditMode = (e) => {
        ...
        setInputValues({
            returnExchangeMemo2: returnExchange?.returnExchangeMemo2,
        });
    }
    ...
    return (
        <>
            <td>
                <div>
                    <div>
                        <ClickableSpan onClick={(e) => handleClickOpenEditMode(e)}>{returnExchange?.returnExchangeMemo2 ? returnExchange?.returnExchangeMemo2 : <i style={{ color: '#606060' }}><u>클릭해서 수정</u></i>}</ClickableSpan>
                        {isLoading &&
                            <FieldCircleLoading
                                progressSize={20}
                            />
                    ...
                    <EditBox>
                        <form onSubmit={(e) => handleSubmit(e)}>
                            <div className='inputBox'>
                                <div>
                                    <textarea type="text" name='returnExchangeMemo2' value={inputValues?.returnExchangeMemo2 || ''} onChange={(e) => handleChangeInputValues(e)} autoFocus />
                                </div>
                            </div>
