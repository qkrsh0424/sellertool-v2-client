...
export function MdAddItem({
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        let newExcelTranslatorUploadHeaderList = excelTranslatorUploadHeaderList ? [...excelTranslatorUploadHeaderList] : [];
        ...
        // 필드 이름 중복 체크
        if (newExcelTranslatorUploadHeaderList?.some(r => r.headerName === inputValue)) {
        ...
        const startIndex = 0;
        const lastIndex = newExcelTranslatorUploadHeaderList?.length - 1;
        ...
        switch (locationType) {
            ...
            default:
                ...
                indexAt = newExcelTranslatorUploadHeaderList.findIndex(r => r.id === selectedManuallyHeaderId);
        ...
        const newItem = { id: uuidv4(), headerName: inputValue, cellType: 'Object', orderNumber: 0 };
        const newList = listUtils.pushAt(newItem, indexAt, newExcelTranslatorUploadHeaderList).map((r, index) => { return { ...r, orderNumber: index } });
    ...
    return (
        <>
            <CustomDialog open={open} onClose={() => onClose()} maxWidth="sm">
                ...
                <CustomDialog.Title>업로드 엑셀 필드를 추가 합니다.</CustomDialog.Title>
                <St.Container>
                        ...
                        <div className='content__box'>
                            ...
                            <div className='locationTypeButtonGroup__box'>
                                ...
                                <CustomBlockButton
                                    ...
                                    disabled={excelTranslatorUploadHeaderList?.length <= 0}
                                >
                                    직접설정
                                </CustomBlockButton>
                            </div>
                            {locationType === LOCATION_TYPE.MANUALLY &&
                                <div className='manually__box'>
                                    ...
                                    >
                                        ...
                                        {excelTranslatorUploadHeaderList?.map((r, index) => {
                                            if (excelTranslatorUploadHeaderList?.length - 1 === index) {
