...
export function useApiHook(props) {
    ...
    const reqCreateAllInventoryReleaseList = async (options = { body: {}, headers: {} }, callbackFn = (results, response) => { }) => {
    ...
    const reqInventoryReleaseBulkCreateExcelUpload = async (options = { formData: null, params: {}, headers: {} }, callbackFn = (results, response) => { }) => {
        await inventoryReleaseDataConnect.inventoryReleaseBulkCreateExcelUpload(options?.formData, options?.headers)
            .then(res => {
                if (res.status === 200) {
                    callbackFn(res?.data?.data, res);
                }
    ...
    return {
        ...
        reqCreateAllInventoryReleaseList,
        reqInventoryReceiveBulkCreateExcelUpload,
        reqInventoryReleaseBulkCreateExcelUpload
    }
}