...
import { EventAppliedLogDataConnect } from "../../../../../../data_connect/EventAppliedLogDataConnect";
...
const eventAppliedLogDataConnect = EventAppliedLogDataConnect();
...
export function useDataSourceHook(props) {
    ...
    const onReqFetchEventAppliedLogList = async (options = {}, callbackFn = (results, response) => { }) => {
        await eventAppliedLogDataConnect.searchList()
            .then(res => {
                if (res?.status === 200) {
                    callbackFn(res?.data?.data, res);
                }
